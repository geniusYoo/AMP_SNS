        ...
        signupBtn.setOnClickListener { // signup 버튼 눌리면
            if(correctLabel.toString().equals("equal password !!")) {
                Firebase.auth.createUserWithEmailAndPassword(id.toString(), password.toString()).addOnCompleteListener {
                    if(it.isSuccessful) {
                        Firebase.auth.signInWithEmailAndPassword(id.toString(), password.toString())

                        val currentUser = Firebase.auth.currentUser
                        val db = Firebase.firestore
                        val user = User(currentUser?.uid, id.text.toString(), nickname.text.toString())

                        if(currentUser != null) {
                            db.collection("user").document(currentUser.uid).set(user).addOnCompleteListener {
